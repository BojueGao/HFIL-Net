...
def train(train_loader, model, optimizer, epoch, save_path):
    ...
    for i, (images, gts, depth) in enumerate(train_loader, start=1):
        ...
        out1, out2, out3 = model(images, depth)
        ...
        if i % 100 == 0 or i == total_step or i == 1:
            print('{} Epoch [{:03d}/{:03d}], Step [{:04d}/{:04d}], LR:{:.7f}, loss1:{:4f}, loss2:{:4f}, loss3:{:4f}'.
                  format(datetime.now(), epoch, opt.epoch, i, total_step,
                         optimizer.state_dict()['param_groups'][0]['lr'], loss1.data, loss2.data, loss3.data))
            logging.info(
                '#TRAIN#:Epoch [{:03d}/{:03d}], Step [{:04d}/{:04d}], LR:{:.7f}, loss1:{:4f}, loss2:{:4f}, loss3:{:4f}, mem_use:{:.0f}MB'.
                format(epoch, opt.epoch, i, total_step, optimizer.state_dict()['param_groups'][0]['lr'],
                       loss1.data, loss2.data, loss3.data, memory_used))
...
def test(test_loader, model, epoch, save_path):
    ...
    with torch.no_grad():
        ...
        for i in range(test_loader.size):
            ...
            res1 = F.upsample(out1, size=gt.shape, mode='bilinear', align_corners=False)
            res1 = res1.sigmoid().data.cpu().numpy().squeeze()
            res1 = (res1 - res1.min()) / (res1.max() - res1.min() + 1e-8)
            mae_sum1 += np.sum(np.abs(res1 - gt)) * 1.0 / (gt.shape[0] * gt.shape[1])

            res2 = F.upsample(out2, size=gt.shape, mode='bilinear', align_corners=False)
            res2 = res2.sigmoid().data.cpu().numpy().squeeze()
            res2 = (res2 - res2.min()) / (res2.max() - res2.min() + 1e-8)
            mae_sum2 += np.sum(np.abs(res2 - gt)) * 1.0 / (gt.shape[0] * gt.shape[1])

            res3 = F.upsample(out3, size=gt.shape, mode='bilinear', align_corners=False)
            res3 = res3.sigmoid().data.cpu().numpy().squeeze()
            res3 = (res3 - res3.min()) / (res3.max() - res3.min() + 1e-8)
            mae_sum3 += np.sum(np.abs(res3 - gt)) * 1.0 / (gt.shape[0] * gt.shape[1])

            mae_sum = (mae_sum1 + mae_sum2 + mae_sum3) / 3
